...
if not api_key:
    logger.warning("GEMINI_API_KEY environment variable is not set. Using mock data for tests.")
    mock_mode = True
else:
    mock_mode = False
...
async def generate_test(request: TestRequest, req: Request, db: Session = Depends(get_db)):
    try:
        ...
        if request.test_id is not None:
            ...
            if existing_test:
                logger.info(f"Using existing test with test_id: {existing_test.test_id}")
                
                # If test already exists, get its questions
                db_questions = QuestionService.get_questions_by_test_id(db, existing_test.test_id)
                ...
                if db_questions and len(db_questions) > 0:
                    ...
                    for q in db_questions:
                        ...
                        correct_answer = next((opt.option_text for opt in q.options if opt.is_correct), None)
        ...
        if mock_mode:
            logger.info(f"Using mock data for skill: {request.skill}, questions: {request.num_questions}")
...
8. Options should be plausible and well-distributed"""
        ...
        existing_questions = QuestionService.get_questions_by_test_id(db, db_test.test_id)
        if not existing_questions or len(existing_questions) == 0:
            ...
            for q_data in test_data["questions"]:
                ...
                try:
                    ...
                    logger.info(f"Created question ID: {db_question.id} for test {db_test.test_id}")
                except Exception as e:
                    logger.error(f"Error creating question: {str(e)}")
                    # Continue with other questions even if one fails
            
            # Log the number of stored questions
            logger.info(f"Successfully stored {len(db_questions)} questions for test {db_test.test_id}")
        else:
            logger.info(f"Test {db_test.test_id} already has {len(existing_questions)} questions, skipping question creation")
    ...
    try:
        ...
        for i, q in enumerate(test_data["questions"]):
            ...
            if q["correct_answer"] not in q["options"]:
                logger.warning(f"Question {i}: correct_answer not in options, fixing...")
...
async def save_snapshot(
    ...
    try:
        # Print debug info
        print(f"Received snapshot request - test_id: {test_id}, type: {snapshot_type}, filename: {image.filename}")
