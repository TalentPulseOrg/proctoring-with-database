/**
 * Microphone Permission Check Feature Module
 * 
 ...
 * 
 * Usage:
 *     import { useMicrophonePermissionMonitor } from './features/proctoring/microphone_permission_check';
 *     
 *     // Use in component
 *     const { hasPermission, status, recheckPermission } = useMicrophonePermissionMonitor(sessionId, isActive);
 */

import { useState, useEffect, useCallback } from 'react';
import { logMicrophonePermissionViolationModular, logMicrophonePermissionGrant } from '../../../api/api';

/**
 * Hook for monitoring microphone permission status
 * @param {number} sessionId - The test session ID
 * @param {boolean} isActive - Whether monitoring is active
 * @returns {object} Microphone permission monitoring state and functions
 */
export const useMicrophonePermissionMonitor = (sessionId, isActive = false) => {
  ...
  const checkPermission = useCallback(async () => {
    try {
      ...
      if (sessionId) {
        await logMicrophonePermissionGrant(sessionId);
      }
      ...
      if (sessionId) {
        await logMicrophonePermissionViolationModular({
...
/**
 * Component for microphone permission warning
 ...
 */
export const MicrophonePermissionWarning = ({ hasPermission, status, onRecheck }) => {
  if (hasPermission) return null;

  return (
    <div className="bg-red-100 border border-red-400 text-red-700 px-4 py-3 rounded relative">
      <strong className="font-bold">Microphone Access Required!</strong>
      <span className="block sm:inline">
        {' '}Please enable microphone access to continue with the test.
      </span>
      <button
        onClick={onRecheck}
...
/**
 * Component for microphone permission status display
 * @param {object} props - Component props
 * @param {boolean} props.hasPermission - Whether microphone permission is granted
 * @param {string} props.status - Permission status
 ...
 */
export const MicrophonePermissionStatus = ({ hasPermission, status, isMonitoring }) => {
  ...
  const getStatusText = () => {
    switch (status) {
      ...
      case 'prompt':
        return 'Microphone Access Pending';
      default:
...
export default {
  useMicrophonePermissionMonitor,
